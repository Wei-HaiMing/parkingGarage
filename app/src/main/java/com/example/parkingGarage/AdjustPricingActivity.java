...
import com.example.parkingGarage.databinding.ActivityAdjustPricingBinding;
...
public class AdjustPricingActivity extends AppCompatActivity {
    ...
    private static final String ADJUST_PRICING_ACTIVITY_USER_ID = "com.example.parkingGarage.ADJUST_PRICING_USER_ID";

    static final String SAVED_INSTANCE_STATE_USERID_KEY = "com.example.parkingGarage.SAVED_INSTANCE_STATE_USERID_KEY";

    private ActivityAdjustPricingBinding binding;
    private User user;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        binding = ActivityAdjustPricingBinding.inflate(getLayoutInflater());
        setContentView(binding.getRoot());
        EdgeToEdge.enable(this);
        repository = ParkingGarageRepository.getRepository(getApplication());
        loginUser(savedInstanceState);
        ViewCompat.setOnApplyWindowInsetsListener(findViewById(R.id.AdjustPricingTextView), (v, insets) -> {
    ...
    private void loginUser(Bundle savedInstanceState) {
        ...
        if(loggedInUserId == LOGGED_OUT){
            loggedInUserId = getIntent().getIntExtra(ADJUST_PRICING_ACTIVITY_USER_ID, LOGGED_OUT);
        }
        if(loggedInUserId == LOGGED_OUT){
            return;
        }
